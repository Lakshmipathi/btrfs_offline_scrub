#!/bin/bash
#
# Raid56: Inject data stripe corruption and fix them using scrub.
# 
# Script will perform the following:
# 1) Create Raid5/Raid6 using 3/4 loopback devices.
...
LAYOUT_TMP=$(mktemp --tmpdir btrfs-progs-raid56-file.layoutXXXXXX)
STRIPEINFO_TMP=$(mktemp --tmpdir btrfs-progs-raid56-file.infoXXXXXX)
PARITYINFO1_TMP=$(mktemp --tmpdir btrfs-progs-raid56-initial.parityXXXXXX)
PARITYINFO2_TMP=$(mktemp --tmpdir btrfs-progs-raid56-end.parityXXXXXX)
...
find_parity_stripe_count(){
	...
	for dev in ${devs[@]}; do
		...
		grep $known_parity_value | awk '{print "'$dev' |" $1}' >> $PARITY_COUNT
	done
}
...
verify_parity_stripe(){
	...
	if [ "$parity_stripe_count_before" != "$parity_stripe_count_after" ];then
		...
		echo "parity files: $PARITYINFO1_TMP $PARITYINFO2_TMP" >> "$RESULTS"
...
#$4 RAID type
test_raid56_datastripe_corruption(){
	filename=$1
	stripe_num=$2
	test_stripe=$3
	raid_type=$4

	if [ $raid_type == "raid6" ];then
		ndevs=4
	fi
	prepare_devices
	dev1=${devs[1]}

	test_mkfs_multi -d $raid_type -m $raid_type
	...
	#cleanup data-stripe layinfo and parity info files
	rm -f $LAYOUT_TMP
	rm -f $STRIPEINFO_TMP
	rm -f $PARITYINFO1_TMP
	rm -f $PARITYINFO2_TMP
}

for type in "raid5" "raid6";do
test_raid56_datastripe_corruption file128k.txt 2 1 $type #file with 2 stripe,corrupt 1st.
test_raid56_datastripe_corruption file192k.txt 3 2 $type #file with 3 stripe,corrupt 2nd.
test_raid56_datastripe_corruption file256k.txt 4 3 $type
test_raid56_datastripe_corruption file512k.txt 8 6 $type
test_raid56_datastripe_corruption file768k.txt 12 10 $type
test_raid56_datastripe_corruption file1m.txt 16 14 $type #1MB file, corrupt 14th stripe.
test_raid56_datastripe_corruption file2m.txt 32 23 $type #2MB file, corrupt 23rd stripe
test_raid56_datastripe_corruption file4m.txt 64 50 $type #4MB file
done
