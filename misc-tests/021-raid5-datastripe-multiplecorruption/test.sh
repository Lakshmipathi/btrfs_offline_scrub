...
#$1 - data stripe number to verify
verify_data_stripe(){
	...
	if [ $value_after_scrub != $stripe_content ];then
		...
		echo "Scrub failed to fix data stripe corruption" >> "$RESULTS"
	else
...
verify_parity_stripe(){
	...
	if [ "$parity_stripe_content" != "$parity_stripe_after_scrub" ];then
		...
		echo "Scrub corrupted parity stripe." >> "$RESULTS"
	else
...
test_raid5_datastripe_corruption(){
	...
	#Inject corruption
	for i in `echo "$test_stripe" | tr "," "\012"`;
	do
		corrupt_data_stripe $i
	done
	#Mount the device and start scrub
#	run_check $SUDO_HELPER btrfs scrub start $TEST_MNT
	run_check $SUDO_HELPER btrfs scrub start --offline $dev1
...
test_raid5_datastripe_corruption file256k.txt 4 2,3
test_raid5_datastripe_corruption file512k.txt 8 1,3,5
test_raid5_datastripe_corruption file768k.txt 12 10,8,6,4
test_raid5_datastripe_corruption file1m.txt 16 14,12,10,8,7,6 #1MB file, corrupt 14th stripe.
test_raid5_datastripe_corruption file2m.txt 32 23,22,21,20,19,18,16 #2MB file, corrupt 23rd stripe
test_raid5_datastripe_corruption file4m.txt 64 34,33,32,31,30,20,18,15,10,8,5 #4MB file
test_raid5_datastripe_corruption file8m.txt 128 100,90,80,70,60,50,40,30,20,10 #8MB file and corrupt 10,20...100stripes
